...
% 1) Inpup data and parameters
clear all
close all
% Ntc,Market_exch,Ramp,Eff_el,Emm_fac,Eff_el_chp1,Eff_el_chp2,Eff_chp_boil,Eff_hob,
% El_chp_ind_hh,El_pro_fix_hh, El_cap_hyd,El_eff_hyd, Hyd_res_dd,Hyd_infl_dd,St_hyd,Cost_hyd_corr,
% F_chp1_r,F_chp2_r,F_boil_r,F_hob_r,Pth_chp
% Wv_seg,Cost_wv,Cost_hyd_coff,St_begin,St_min, Cost_p,Cost_r,Cost_seg,Name_r
% 1.1) Input data
td_1=1;                % The beginning day of the analysis
td_n=15;               % The last day of the analysis
n=6;                   % Number of regions to be simulated
Region_sys=1:5;        % The regions included in one system price calculations
...
% 1.2) Visualization and saving names
...
%---------------------------------------------------------------------------------
% 1.3) Loading required classes
id=f_inputdata_r('Input_data.mat',n);
th_1=24*(td_1-1)+1;    
th_n=24*td_n;
esm=f_esm_dd_r(n,th_1,th_n);
%------------------------------------------------------------------------
...
% [Ntc,Market_exch,El_dem_tot,El_dem_res_tot,Ramp,Eff_el,Emm_fac,Eff_el_chp1,Eff_el_chp2,Eff_chp_boil,Eff_hob,....
...
%     Cost_chp1_pool,Cost_chp2_pool,Cost_p,Cost_r,Cost_seg,Name_pool,Name_r]=f_esm_dd_r(n,th_1,th_n);
% clear f_esm_dd_r
tic

NTC=repmat(id.Ntc,1,1,nh); % LATER: NTC should be imported as hourly network capacities from NordPool
n_dd=size(id.Hyd_infl_dd,1);   % Number of the days in the examined year
%-----------------------------------------------------------------------
% 2.2) External power market flows
% Power exchanges between NordPool and other exchanges
Exch_fi_ru=id.Market_exch(:,1);  %MWh/h
Exch_fi_ee=id.Market_exch(:,2);
Exch_se_de=id.Market_exch(:,3);
Exch_dk1_de=id.Market_exch(:,4);
Exch_dk2_de=id.Market_exch(:,5);
Exch_no_nl=id.Market_exch(:,6);
Exch_uk=id.Market_exch(:,7);
...
%--------------------------------------------------------------------------------------
El_dem_pool=esm.El_dem_res_tot+El_dem_exch;
El_dem_pool_uk=esm.El_dem_res_tot(:,6)+El_dem_exch(:,6);
%-------------------------------------------------------------
% 2.3) Hydropower initialization 
np=length(esm.Name_pool);
Hyd_res_d=zeros(n_dd,n);
Hyd_infl_d=zeros(n_dd,n);
Hyd_res_d(:,Region_wv)=id.Hyd_res_dd;
Hyd_infl_d(:,Region_wv)=id.Hyd_infl_dd;
...
Stor_pre(:,1)=id.St_begin;             % Resrvoir level at the beginning of the first day 
...
Ramp_sort=cell(n,nh);                               % The sorted ramping constraints
...
El_exp_crit_tech= El_exp_crit_dom+repmat((sum(id.Ntc,2))',nh,1);     %NOTICE: Ntc should be corrected if NTC is used                        % Critical excess assuming full-export possibilities: demand after fixed power, fixed import, and domestic VRE
...
N_week=zeros(52,1)+7;
f = @(k) repmat(id.Cost_hyd_corr(k,:), round(N_week(k)), 1);
...
El_eff_hyd=id.Eff_el(5,:);
El_cap_hyd=id.P_pow(5,:);
...
for d=1:nd              % d: count of day   
    ...
    for r=Region_wv      % Regions of hydropower water value
        Hyd_reg_infl(r,d)=Hyd_infl_d(dc,r)-sum(esm.El_pro_vre4(h1:h2,r))/El_eff_hyd(r);   % IMPLEMENT EFFICIENCY IN OTHER INTERFACES
    
        [Hyd_mar_d, Cost_mod, stor_rem]=f_hydroplan(id.Cost_wv,id.Wv_seg,id.Cost_hyd_coff(:,r)...
            ,Hyd_reg_infl(r,d),Stor_pre(r,d),id.St_min(r),Hyd_unaccepted(r,d),...
        El_dem_pool(h1:h2,r),El_pro_vre(h1:h2,r),...
        El_cap_hyd(r),id.St_hyd(r),El_eff_hyd(r));
        clear f_hydroplan
   ...
   %------------------------------------------------------------------------------------------------ 
    ...
    for h=24*(d-1)+1:24*d                       % 24 h: hour of the day 
        ...
        Pow_pool=[esm.El_pro_vre1(h,:); esm.El_pro_vre2(h,:); esm.El_pro_vre3(h,:);...
            esm.El_pro_vre4(h,:) ;squeeze(Hyd_pool_d(k,:,:)); esm.El_pro_nuc(h,:);...
            esm.El_chp1_mar(:,:,h);esm.El_chp2_mar(:,:,h);esm.El_ppchp_mar(:,:,h)];
        % Matrix of Ramping constraints (later the ramps can be region specific)
        Ramp_pool = [id.Ramp(1:4,:);repmat(id.Ramp(5,:),length(Ind_hyd),1);id.Ramp(6:end,:)]; 
        ...
        for r=1:n
         % PAY ATTENTION: the order of the cost elements in the vector below MUST conform to the order of power production mix in "Pow_pool" (no need for sorting based on the costs)          
            Cost_mar{r,h}=[id.Cost_p(1:4,r)' Cost_hyd_wv(r,:).*id.Cost_p(5,r)...
                id.Cost_p(6,r)' esm.Cost_chp1_pool(:,r)' esm.Cost_chp2_pool(:,r)' id.Cost_p(7:end,r)'];
            Pow_mar{r,h}=Pow_pool(:,r);
            Cost1=Cost_mar{r,h};
            Ramp1=Ramp_pool(:,r);                              % Ramping rates per technology (no time dimension)
        ...
        [Sup_uk,Cost_uk ]=suppcurve_dis(A1, Cost_sort{6,h}, id.Cost_r(6),id.Cost_seg(6));
    ...
    [Area_p0,Sup_tot, Exch_opt,Ind_last,Sup_last,Sys_p0]=f_np_dd_r(El_dem_pool(h1:h2,:)', Pow_sort(:,h1:h2),...
    Cost_sort(:,h1:h2),Ramp_sort(:,h1:h2),NTC(:,:,h1:h2),id.Cost_r,id.Cost_seg,Region_sys,peak_price);
    clear f_np_dd_r
    %----------------------------------------------------------------------------------- 
    ...
    for h=24*(d-1)+1:24*d
        ...
        for r=1:n
              ...
              Pow_act(r,:,h)=P_mode;
              El_pro_hyd(h,r)=sum(Pow_act(r,Ind_hyd,h))+esm.El_pro_vre4(h,r);         
...
Name_table=cell(length(esm.Name_pool)+1,1);
Name_table(1:9)=esm.Name_pool(1:9);Name_table{10}='(Total hydro)';
Name_table(11:end)=esm.Name_pool(10:end);
P_act_hyd=array2table(fix([Pow_act(:,1:9,h)'; sum(Pow_act(:,4:9,h),2)'; Pow_act(:,10:end,h)']),...
    'RowNames',Name_table,'VariableNames',id.Name_r);
...
El_act_ppchp1=min(Pow_act(23:28,:,:),esm.El_ppchp2_mar);
...
% District heating
dh_pro_chp=sum(esm.Dh_pro_chp)./1e6; 
dh_pro_boil=sum(esm.Dh_pro_boil)./1e6;
dh_pro_hob=sum(esm.Dh_pro_hob)./1e6;
...
el_dem_tot=sum(esm.El_dem_tot)./1e6;
el_imp_fix=sum(-El_dem_exch)./1e6;                  % Fixed or simulated import from countries outside Nordpool
el_pro_fix=sum(id.El_pro_fix_hh)./1e6;                 % Fixed domestic production
...
f_nuc=el_act_nuc./id.Eff_el(6,:);
F_pp_a=El_act_pp_a./id.Eff_el(7:12,:);

F_chp1_a=El_act_chp1_a./id.Eff_el_chp1;
F_chp2_a=El_act_chp2_a./id.Eff_el_chp2;

f_boil=dh_pro_boil./id.Eff_chp_boil;
f_hob=dh_pro_hob./id.Eff_hob;

F_boil_a=repmat(f_boil,size(id.F_boil_r,1),1).*id.F_boil_r;
F_hob_a=repmat(f_hob,size(id.F_hob_r,1),1).*id.F_hob_r;
...
dh_pro_chp_vre=el_act_chp_vre./id.Pth_chp;
...
F_chp1_vre=(repmat(el_act_chp1_vre,size(id.F_chp1_r,1),1).*id.F_chp1_r)./id.Eff_el_chp1;
F_chp2_vre=(repmat(el_act_chp2_vre,size(id.F_chp2_r,1),1).*id.F_chp2_r)./id.Eff_el_chp2;
f_boil_vre=dh_pro_boil_vre./id.Eff_chp_boil;
F_boil_vre=repmat(f_boil_vre,size(id.F_boil_r,1),1).*id.F_boil_r;
...
X=th_1:th_n;
figure
plot(X',esm.El_dem_tot(:,fig_r)+El_dem_exch(:,fig_r),'LineWidth',0.3,'Color','black');hold on;xlim([th_1 th_n]);           % The name should be adjusted based on each desirable production mode
area(X',[El_act_nuc(:,fig_r) El_pro_hyd(:,fig_r),...
      id.El_pro_fix_hh(th_1:th_n,fig_r) El_act_chp1(:,fig_r)+El_act_chp2(:,fig_r),...
     esm.El_pro_vre2(:,fig_r)+esm.El_pro_vre3(:,fig_r) esm.El_pro_vre1(:,fig_r),...
