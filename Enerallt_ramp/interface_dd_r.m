...
fig_r=5;                    % Number of region to see some figures of the results for
h=24*4+10;                  % A sample hour to see the table results
%---------------------------------------------------------------------------------
% 1.1) Input data
td_1=1;                % The beginning day of the analysis
td_n=31;               % The last day of the analysis
n=1;                   % Number of regions to be simulated
...
for d=1:nd              % d: count of day   
   ...
   %------------------------------------------------------------------------------------------------ 
    ...
    for h=24*(d-1)+1:24*d                       % 24 h: hour of the day 
        ...
        Ramp_pool = [Ramp(1:4,:);repmat(Ramp(5,:),length(Ind_hyd),1);Ramp(6:end,:)]; 
    ...
    Cost_sort(:,h1:h2),Ramp_sort(:,h1:h2),NTC(:,:,h1:h2),Cost_r,Cost_seg,Region_sys,peak_price);
    clear f_np_dd_r
    %----------------------------------------------------------------------------------- 
