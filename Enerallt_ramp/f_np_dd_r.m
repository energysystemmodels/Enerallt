...
%----------------------------------------------------------------------------
function [Area_p,Sup_tot, Exch_opt,Ind_last,Sup_last,Sys_p]=f_np_dd_r(Demand,Pow0,Cost0,Ramp0,Ntc,Coff,Num_cost,Region_sys,peak_price)
...
decim=1;                           % Making costs bigger to avoid decimals (optional)
...
for i=1:n
  for t=1:nh
      ...
      Pow1{i,t}=reshape(repmat((Pow0{i,t}/Num_cost(i))',1,Num_cost(i))',1,length(Pow0{i,t})*Num_cost(i));
      Cost1{i,t}=fix(decim.*(Cost1{i,t}));     % MUST BE RETURNED LATER (Make them 10 times bigger and get rid of deciamls)
      Ramp1{i,t}=reshape(repmat(Ramp0{i,t},1,Num_cost(i))',1,length(Ramp0{i,t})*Num_cost(i));
  end
end
%-------------------------------------------------------------------------------------------------
...
[Sup_tot,Exch_opt0,sys_cost]=f_opt_mar_dd_r(Demand,Pow1,Cost1,Ramp1,Ntc);
Exch_opt=Exch_opt0;
clear f_opt_mar_dd
%---------------------------------------------------------------------------------------------------------
